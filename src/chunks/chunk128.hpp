...
class ByteChunk128{
  friend class ByteChunk256;

  types::iarr<chars_per_chunk128> bytes = {};

  protected:
  void basic_constructor(Bytearray bytes){
    if (bytes.length() > chars_per_chunk128){
      throw std::invalid_argument("Input array dimension is bigger than "+std::to_string(chars_per_chunk128)+", got "+std::to_string(bytes.length()));
    }
    if (bytes.length() < chars_per_chunk128){
      throw std::invalid_argument("Input array dimension is smaller than "+std::to_string(chars_per_chunk128)+", got "+std::to_string(bytes.length()));
    }
  ...
  ByteChunk128(types::iarr<chars_per_chunk128> bytes){
    types::ilist vct = {};
  ...
  types::balist get_rows(){
    ...
    for (int chunk_row_idx = 0; chunk_row_idx < chunk128_side; chunk_row_idx++){
      Bytearray row = {};
      // scorri del chunk128_side per prendere il valore della riga (2, 6, 10, 14)
      for (int chunk_idx = chunk_row_idx; chunk_idx < chars_per_chunk128; chunk_idx += chunk128_side){
  ...
  Bytearray get_row(int idx){
    return this->get_rows()[idx < 0? chunk128_side + idx : idx];
  }
  ...
  void set_row(int row_idx, Bytearray value){
    int count = 0;
    if (row_idx < 0){
      row_idx = chunk128_side + row_idx;
    }
    for (int i = row_idx; i < chars_per_chunk128; i += chunk128_side){
  ...
  types::balist get_columns(){
    ...
    for (int chunk_column_idx = 0; chunk_column_idx < chars_per_chunk128; chunk_column_idx += chunk128_side){
      Bytearray column = {};
      // scorri di 1 per prendere il valore della colonna (4, 5, 6, 7)
      for (int chunk_idx = chunk_column_idx; chunk_idx < chunk128_side+chunk_column_idx; chunk_idx++){
  ...
  Bytearray get_column(int idx){
    return this->get_columns()[idx < 0? chunk128_side + idx : idx];
  }
  ...
  void set_column(int column_idx, Bytearray value){
    int count = 0;
    if (column_idx < 0){
      column_idx = chunk128_side + column_idx;
    }
    for (int i = column_idx*chunk128_side; count < chunk128_side; i++){
      this->operator[](i) = value[count];
      count++;
    }
  }
  ...
  int get_first_null(){
    int acc = -1;
    for (int i = chars_per_chunk128-1; i >= 0; i--){
  ...
  int length(){
    ...
    return idx == -1? chars_per_chunk128 : idx;
  }
  int size(){
    return this->length();
  }
  int padding(){
    return chars_per_chunk128-this->length();
  }

  void push_back(int x){
    int idx = this->get_first_null();
    if (idx == -1){
      throw std::overflow_error("Trying to push back an element into an already full chunk");
    }
    if (idx != -1){
      this->operator[](idx) = x;
    }
  ...
  void extend(Bytearray bytes){
    int idx = this->length();
    if (bytes.length() > chars_per_chunk128 - idx){
      throw std::invalid_argument("Input array exceed free chunk dimension");
    };
    for (int i = idx; i < chars_per_chunk128; i++){
      if (i - idx == bytes.length()) return;
      this->operator[](i) = bytes[i - idx];
    }
  }

  types::iarr<chars_per_chunk128>::iterator begin(){
    return this->bytes.begin();
  }
  types::iarr<chars_per_chunk128>::iterator end(){
    return this->bytes.end();
  }

  int& operator[](int idx){
    return this->bytes[idx >= 0? idx : chars_per_chunk128 + idx];
  }
  ...
  ByteChunk128 shift_rows_left() {
    ByteChunk128 result;
    for (int row_idx = 0; row_idx < chunk128_side; row_idx++){
